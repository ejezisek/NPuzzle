...
public class PatternDBSlidingSquareNode  implements Node<PatternDBSlidingSquareNode>{
	...
	public static byte[] assignBytes(Integer i, PatternDBSlidingSquareTriePrototype prototype)
	{
		byte[] bytes=prototype.getTrie().toBytes(i);
		return bytes;
	}
	public PatternDBSlidingSquareNode(Integer i, PatternDBSlidingSquareTriePrototype flyweight)
	{
		this(new PatternDBSlidingSquareUsingLong(assignBytes(i, flyweight), flyweight.get(i), flyweight.getHeight(), flyweight.getWidth(), flyweight.availableValues));
	}
