...
public class Main {
    ...
    public static  Boolean BFS(PatternDBSlidingSquareNode head, Action<PatternDBSlidingSquareNode, Boolean> action) throws IOException
    {
    	...
    	Set<PatternDBSlidingSquareNode> toCheck=new HashSet<PatternDBSlidingSquareNode>();
    	contain.add(head);
    	q.add(head);
    	int currentLength=1;
    	while(true)
    	{
    		if(q.isEmpty())
    		{
				...
				{
					PatternDBSlidingSquareNode [] arr=new PatternDBSlidingSquareNode[toCheck.size()];
    				long [] vals=new long[toCheck.size()];
    				int i=0;
    				for(PatternDBSlidingSquareNode pd:toCheck)
    				{
    					vals[i]=pd.getSquares();
    					arr[i]=pd;
    					i++;
    				}
    				...
    				{
    					if(!results[i] && !contain.contains(arr[i]))
    					{
        					contain.add(arr[i]);
        					if(dbQueue)
        					{
    		...
    		{
    			if(currentLength==val.getNumInRoute())
    			{
    				toCheck.add(val);
    			}
    			else
    			{
    				System.out.println(currentLength + " " + val.getNumInRoute());
    			}
    			if(toCheck.size()>checkMaxSize || val.getNumInRoute()!=currentLength)
    			{
    				PatternDBSlidingSquareNode [] arr=new PatternDBSlidingSquareNode[toCheck.size()];
    				long [] vals=new long[toCheck.size()];
    				int i=0;
