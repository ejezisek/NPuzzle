...
function refresh()
{
	...
	{
		layer=nodes[i];
		layer.visitedLeft=null;
		layer.visitedRight=null;
		layer.fillStyle=initialFill;
	}
	nodeStack.push(head);
}
...
function traverse()
{
	...
	if(nodeStack.length>0)
	{
		...
		$('canvas').drawLayers();
		numCanTransmit++;
		if(val.visitedRight!=null || val.visitedLeft!=null || (val.left==null && val.right==null))
		{
			val.fillStyle=activeFill;
			val.visitedLeft=null;
			val.visitedRight=null;
			$('canvas').drawLayers();
			return;
		}
		else
		...
		if(val.right!=null)
		{
			val.visitedRight=true;
			nodeStack.push(val.right);
		}
		if(val.left!=null)
		{
			val.visitedLeft=true;
