...
public class PatternDBSlidingSquareUsingLong
{
	...
	public PatternDBSlidingSquareUsingLong(long value, int numInRoute, byte height, byte width, int[] availableValues)
	{
		this.availableValues=availableValues;
		this.squares=value;
		this.numInRoute=(byte)numInRoute;
		this.height=height;
		this.width=width;
	...
	public PatternDBSlidingSquareUsingLong(byte[] value, int numInRoute, byte height, byte width, int[] availableValues)
	{
		this(toLong(value), numInRoute, height, width, availableValues);
	}

	/**
	 * Creates an ixj square with the elements in random spots.
	 */
	public PatternDBSlidingSquareUsingLong(byte height, byte width, int[] availableValues)
	{
		this(height, width,new byte[]{1,2,3,4,6,6,6,6,6,6,6,6,6,6,6,0}, availableValues);
	}
	
	/**
	 * Creates an ixj square with the elements in random spots.
	 */
	public PatternDBSlidingSquareUsingLong(byte height, byte width, byte[]vals, int [] availableValues)
	{
		this(vals, 1, height, width, availableValues);		
	}
	...
	{
		numInRoute=(byte) (ms.numInRoute);
		this.height=ms.height;
		this.width=ms.width;
		emptyPosition=new Point(ms.emptyPosition);
		squares=ms.squares;
		this.availableValues=ms.availableValues;
	}
	...
	{
		long bitPosVal=(long)(16-1)<<(pos*4);
	...
	private void swap(Point one, Point two)
	{
		...
		{
			byte tempVal=(byte)val;
			if(tempVal!=0 && (tempVal==temp || tempVal==temp2))
			{
				numInRoute++;
			}
		}
	...
	private static long toLong(byte[] squares)
	{
		long sum=0;
		int index=0;
		for(byte v:squares)
